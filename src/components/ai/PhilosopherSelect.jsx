...
export default function PhilosopherSelect({onSelectPhilosopher}) {
    const philosophers = [
        ...
        },
        {
            ...
            model: 'descartes',
            introduce:['모든 것을 의심하라', '그 속에서 진리와 나의 존재를 찾으리라.']
        },
        {
            name: '아리스토텔레스',
            description: [
        ...
        },
        {
            ...
            model: 'plato',
            introduce : ['이상적인 세상은 우리의 상상 속에 존재하며','그곳에서 진정한 정의를 찾는다.']

        },
    ];

    return(
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-screen-xl mx-auto w-full px-4">
                    {philosophers.map((philosopher, index) => (
                        
                        <div key={index} className="flex justify-center cursor-pointer" onClick={() => onSelectPhilosopher(philosopher)}>
                            <div className="border-4 border-black bg-teal-400 p-4 rounded-lg w-80 h-128 flex items-center 
                                hover:bg-teal-500 hover:opacity-90 active:bg-teal-600 active:opacity-80 transition ease-in-out duration-200">
                                <PhilosopherCard
