...
import { sendTempPassword, verifyTempPassword } from "@/api/user/user"; // API 함수 임포트
...
const PasswordFind = () => {
  ...
  const handleSendTempPassword = async () => {
    try {
      const response = await sendTempPassword({ userId, email });
  ...
  const handleVerifyTempPassword = async () => {
    try {
      const response = await verifyTempPassword({ userId, tempPassword });
      setMessage(response.data.message);
      openModal();
    } catch (error) {
