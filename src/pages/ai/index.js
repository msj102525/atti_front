...
export default function Ai() {
    ...
    let thirdText = ["마음에 들지 않으면", "녹음버튼을 눌러 다시 녹음하거나", "텍스트를 수정해보세요!"]
    let fourthText = ["본인의 현재 감정이 어떤지 궁금하시다면", "아래 '내감정은?' 버튼을 눌러보세요 !"]

    let GuideMessage = [initialText, secondText, thirdText, fourthText];
        ...
        <div>
            ...
            <div className="min-h-screen w-full flex flex-col items-center py-10">
                ...
                {!isPhilosopherSelected ? (
                    ...
                    <PhilosopherConsult philosopher={selectedPhilosopher} setMessageSequence={setMessageSequence}/>
                )}
            </div>
            <Footer />
        </div>
